...
include "koneksi.php";
    ...
    $sqluser = mysqli_query($conn,"SELECT * FROM tbl_user WHERE codename = '$iduser'");
...
<body>
    ...
    <div class="panel panel-default" style="width: 75%; margin:auto; margin-top:30px; margin-bottom:30px;">
                       ...
                       <form action="action.php?aksi=edituser&amp;id=<?= $iduser ?>" method="post" enctype="multipart/form-data" autocomplete="off" style="width:75%; margin:auto;">
                            <div class="form-group">
                                <label for="id">Codename</label>
                                <input type="text" class="form-control" name="codename" required value="<?=$usr['codename']?>"/>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <input type="password" class="form-control" name="password" required value="<?=$usr['pass']?>"/>
                            </div>
                            <div class="form-group">
                                            <label>Level</label>
                                            <select class="form-control" name="level" required value="<?=$usr['level']?>">
                                                <option>1</option>
                                                <option>2</option>
                                            </select>
                            </div>
                                <button type="user" class="btn btn-default">Submit</button>
                            </form>
                            </div>
                            </div>
